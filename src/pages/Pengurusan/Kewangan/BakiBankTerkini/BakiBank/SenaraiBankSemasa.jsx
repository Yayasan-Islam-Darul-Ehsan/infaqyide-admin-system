...
SenaraiBankSemasa.propTypes = {
    
};

function SenaraiBankSemasa(props) {
    ...
    const GET__LIST__BANK = async () => {
        set_loading(true)
        let api = await API(`kewangan/bank-semasa/senarai-akaun?page=${currentPage}&limit=${rowsPerPage}`, {}, "GET", true)
        console.log("Log Api Get List Bank Semasa : ", api)
    ...
    const SEARCH__DATA = (search_string = "") => {
        ...
        if(root_data.length > 0) {
            ...
            let filtered_data = root_data.filter(
                item => 
                    item.bbca_account_bank_name.toLowerCase().indexOf(search_string.toLowerCase()) !== -1 ||
                    item.bbca_account_holder_name.toLowerCase().indexOf(search_string.toLowerCase()) !== -1 ||
                    item.bbca_account_number.toLowerCase().indexOf(search_string.toLowerCase()) !== -1
            )
        ...
        <div>
            <section>
                <div className='flex flex-row justify-between items-center gap-4'>
                    <div>
                        <p className={`font-semibold text-gray-900 ${width <= breakpoints.md ? 'text-sm' : 'text-2xl'}`}>Senarai Akaun Bank Semasa</p>  
                        <p className={`text-sm text-gray-500`}>Berikut adalah senarai maklumat akaun bank semasa yang telah anda daftar. Klik pada senarai di bawah untuk melihat keterangan akaun bank dengan lebih lanjut.</p>  
                    </div>
                    <div>
                        <Button className='bg-emerald-600 text-white' onClick={() => navigate("/kewangan/daftar-bank-semasa")}>Tambah Bank Semasa</Button>
                    </div>
                </div>
            </section>
                                        ...
                                        <tbody className='text-sm p-3'>
                                            {
                                                list_bank.length < 1 && (
                                                    <tr className='border border-gray-100 p-3'>
                                                        <td colSpan={7} className='p-3 text-center'>Anda tidak mempunyai senarai akaun bank semasa buat masa sekarang.</td>
                                                    </tr>
                                                )
                                            }
                                            {
                                                list_bank.length > 0 && list_bank.map((data, index) => (
                                                    <tr key={index} className='border border-gray-100 p-3'>
                                                        ...
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <p className='font-normal text-gray-900'>{data.bbca_account_bank_name}</p>
                                                        </td>
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <p className='font-normal text-gray-900'>{data.bbca_account_holder_name}</p>
                                                        </td>
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <p className='font-normal text-gray-900'>{data.bbca_account_number}</p>
                                                        </td>
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <p className='font-normal text-gray-900'>{parseFloat(data.bbca_balance_amount).toFixed(2)}</p>
                                                        </td>
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <Badge className=' justify-center items-center bg-emerald-600 text-white'>Aktif</Badge>
                                                        ...
                                                        <td width={'10%'} className='p-3 font-normal flex w-full justify-center items-center'>
                                                            <button onClick={() => navigate(`/kewangan/maklumat-bank-semasa`, { state: data })} className='py-3 px-2'>
...
export default SenaraiBankSemasa;