...
SenaraiTransaksiPenerimaan.propTypes = {
    
};

function SenaraiTransaksiPenerimaan(props) {
    ...
    const GET__TRANSACTION = async () => {
        set_loading(true)
        let api = await API(`kewangan/senarai-transaksi-perolehan?page=${currentPage}&limit=${rowsPerPage}`, {}, "GET")
        ...
        <div>
            <section>
                <div className='flex flex-row justify-between items-center gap-4'>
                    <div>
                        <p className={`font-semibold text-gray-900 ${width <= breakpoints.md ? 'text-sm' : 'text-2xl'}`}>Senarai Transaksi Rekod Penerimaan</p>  
                    ...
                    <div>
                        <Button className='bg-teal-600 text-white' onClick={() => navigate("/perolehan/tambah-rekod-transaksi")}>Tambah Rekod Penerimaan</Button>
                    </div>
                </div>
            </section>
            ...
            <section className='mt-6'>
                <Card>
                    <div className='flex justify-between items-center'>
                        <div>
                            <p className='font-semibold text-gray-900 text-lg'>Senarai Transaksi Penerimaan</p>
                            <p className='font-normal text-gray-600 text-sm'>Berikut adalah senarai transaksi penerimaan oleh bank-bank anda.</p>
                        </div>
                    </div>
                    <div className='mt-6'>
                        {
...

                        {
                            loading === false && list_trans.length < 1 && (
                                ...
                                <div className='mt-6 flex justify-center items-center'>
                                    <p className='font-normal text-gray-600 text-sm'>Anda tidak mempunyai transaksi rekod penerimaan oleh bank anda semasa buat masa ini.</p>
                                </div>
                                </>
                            )
                        }
                                        ...
                                        <tbody className='text-sm p-3'>
                                            {
                                                list_trans.length > 0 && list_trans.map((data, index) => (
                                                    <tr key={index} className='border border-gray-100 p-3'>
                                                        ...
                                                        <td width={'10%'} className='p-3 font-normal text-sm text-clip'>
                                                            <Button className='' onClick={() => navigate("/perolehan/maklumat-transaksi", { state: data })}>
...
export default SenaraiTransaksiPenerimaan;