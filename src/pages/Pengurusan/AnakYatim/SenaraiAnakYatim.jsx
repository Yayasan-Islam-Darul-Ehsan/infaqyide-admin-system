...
SenaraiAnakYatim.propTypes = {
    
};

function SenaraiAnakYatim(props) {

    const navigate                                      = useNavigate()
    const { width, breakpoints }                        = useWidth()

    const [loading, set_loading]                        = useState(true)
    const [senarai_data, set_senarai_data]              = useState([])
    ...
    const fetch_anak_yatim = async (offset, limit) => {
        set_loading(true)
        let api = await API(`kariah/anak-yatim/list?page=${offset}&limit=${limit}`, {}, "GET", true)
        if(api.status_code === 200) {
            set_senarai_data(api.data.row)
    ...
    const SEARCH__DATA = (search_string = "") => {
        ...
        if(root_data.length > 0) {
            ...
            set_senarai_data(filtered_data)
        }

        if(search_string == "" || search_string == null) {
            set_senarai_data(root_data)
        }
        
    }

    useEffect(() => {
        fetch_anak_yatim(currentPage, rowsPerPage)
        ...
        <div>
            <section>
                <div className='flex flex-row justify-between items-center gap-4'>
                    <div>
                        <p className={`font-semibold text-gray-900 ${width <= breakpoints.md ? 'text-sm' : 'text-2xl'}`}>Senarai Maklumat Anak Yatim</p>  
                        <p className={`text-sm text-gray-500`}>Berikut adalah senarai anak yatim yang telah didaftarkan di bawah pengurusan kariah anda. Klik pada senarai anak yatim di bawah untuk keterangan lebih lanjut.</p>  
                    </div>
                    <div className='flex flex-row gap-3'>
                        <Button onClick={() => navigate("/anak-yatim/create")} className='bg-teal-600 font-medium text-sm text-white'>Tambah Maklumat Anak Yatim</Button>
                    </div>
                </div>
            </section>  
                            ...
                            !loading && (
                                <>
                                    <div className='flex items-center justify-between'>
                                        <div>
                                            <p className='text-gray-500 text-sm'>Papar {senarai_data.length} per {rowsPerPage} rekod.</p>
                                        </div>
                                        <div>
                                            <Pagination
                                    ...
                                    <div className='mt-3'>
                                        <table className='min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700'>
                                            <thead className="bg-slate-200 dark:bg-slate-700 p-3 rounded-md">
                                                ...
                                                <td width={'10%'} className='p-3 font-semibold text-sm'>Nama Anak Yatim</td>
                                            ...
                                            <tbody className='text-sm p-3'>
                                                {
                                                    senarai_data.length < 1 && (
                                                        <tr className='border border-gray-100 p-3'>
                                                            <td colSpan={7} className='p-3 text-center'>Anda tidak mempunyai senarai anak yatim buat masa sementara waktu.</td>
                                                        </tr>
                                                    )
                                                }

                                                {
                                                    senarai_data.length > 0 && senarai_data.map((data, index) => (
                                                        <tr key={index} className='border border-gray-100 p-3'>
                                                            ...
                                                            <td width={'10%'} className='p-3 font-normal flex w-full justify-center items-center'>
                                                                <button onClick={() => navigate("/anak-yatim/detail", { state: data })} className='py-3 px-2'>
...
export default SenaraiAnakYatim;