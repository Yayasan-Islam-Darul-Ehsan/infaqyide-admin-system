...
KreditPengguna.propTypes = {
    
};

function KreditPengguna(props) {
    ...
    const getData = async (string = "") => {
        set_loading(true)
        try {
            let api = await SYSADMIN_API(`pengurusan/kredit/pengguna?page=${page}&limit=${limit}&search=${string}`, {}, "GET", true)
    ...
    return (
        <div>
            <section>
                <div className="flex justify-between flex-wrap items-center mb-6">
                    <p className="font-medium lg:text-2xl text-xl capitalize text-slate-900 inline-block ltr:pr-4 rtl:pl-4">
                        {`Senarai Maklumat Akaun Kredit Pengguna InfaqYIDE`}
                    </p>
            ...
            <section className='mt-6'>
                <Card title={"Senarai Akaun Kredit Pengguna"} subtitle={"Klik pada senarai akaun kredit pengguna di bawah untuk melihat maklumat terperinci."}>
                                ...
                                (!loading && data.total === 0) && (
                                    <Table.Row flex={1} textAlign="center">
                                        <Table.Cell flex={1} fontSize="small" colSpan={9}>
                                            <td className='w-full'>
                                                Tiada senarai maklumat akaun kredit pengguna buat masa ini.
                                            </td>
                                        </Table.Cell>
                                    </Table.Row>
                                )
                            }
                            {
                                (!loading && data.total > 0) && data.row.map((item, index) => (
                                    <Table.Row key={index} className='p-3' alignItems="center">
                                        ...
                                        <Table.Cell fontSize="small" className='line-clamp-1'>{item.account_fullname || <span className='text-red-600'>-- tiada maklumat --</span>}</Table.Cell>
                                        <Table.Cell fontSize="small" className='line-clamp-1'>{item.credit_account_no || <span className='text-red-600'>-- tiada maklumat --</span>}</Table.Cell>
                                        <Table.Cell fontSize="small" flex="1">
                                            <p className='mx-auto'>{Intl.NumberFormat("ms-MY", {style:'currency',currency:'myr'}).format(item.credit_account_balance)}</p>
                                        </Table.Cell>
                                        <Table.Cell fontSize="small" flex="1">
                                            <p className='mx-auto'>{Intl.NumberFormat("ms-MY", {style:'currency',currency:'myr'}).format(item.credit_account_float)}</p>
                                        </Table.Cell>
                                        <Table.Cell fontSize="small">{item.credit_account_created_date ? moment(item.credit_account_created_date).format("DD MMM YYYY, hh:mm A") : "--"}</Table.Cell>
                                        <Table.Cell flexBasis={150} flexShrink={0} flexGrow={0} fontSize="small">{
                                            item.credit_account_status == "1" ? 
