...
const schema = yup
	.object({
		username: yup
			.string()
			.min(8, "ID pengguna perlu mempunyai minimum 8 aksara")
			.max(20, "ID Pengguna tidak boleh lebih 20 aksara")
			.required("Sila masukkan ID pengguna anda!"),
		email: yup
			.string()
			.email("Invalid email")
			.required("Sila masukkan email anda!"),
		password: yup
			.string()
			.min(8, "Kata laluan perlu mempunyai minimum 8 aksara")
			.max(20, "Kata laluan tidak boleh lebih 20 aksara")
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
			...
			{/* <Button
				...
				isLoading={loading}
			/> */}
			<button disabled={loading} type="submit" className="btn bg-gradient-to-br from-green-600 to-blue-600 text-white block w-full text-center">
				Daftar Institusi
			</button>
		</form>
	);
