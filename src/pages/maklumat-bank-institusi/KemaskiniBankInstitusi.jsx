...
function KemaskiniBankInstitusi(props) {
    ...
    const [record_id, set_record_id]                        = useState(state.org_bank_acc_id)
    const [bank_id, set_bank_id]                            = useState(state.BankID ?? "")
    const [bank_name, set_bank_name]                        = useState(state.org_bank_name ?? "")
    const [bank_holder_name, set_bank_holder_name]          = useState(state.org_acc_holder_name ?? "")
    const [bank_account_number, set_bank_account_number]    = useState(state.org_bank_acc_number ?? "")
    ...
    const UPDATE__PERBANKAN = async () => {
        ...
        if(!bank_name) {
            ...
            let api = await API("updateBankInstitusi", json)
            console.log("Log Function Update Institusi Bank : ", api)
            set_loading(false)

            if(api.status === 200 || api.status_code === 200) {
                toast.success("Maklumat perbankan telah berjaya dikemaskini.")
                setTimeout(() => {
                    navigate(-1)
                }, 1000);
            }
        ...
        <div>
            ...
            footerContent={(
                ...
                <div className='flex justify-end items-center'>
                    ...
                    <Button className='bg-teal-600 text-white' onClick={UPDATE__PERBANKAN}>Ya</Button>
                </div>
                </>
            )}
            >
                <p className='text-sm text-gray-600'>Anda pasti untuk mengemaskini maklumat perbankan ini?</p>
            </Modal>
            ...
            <section className='mt-6'>
                <Card>
                    <div>
                        <p className='text-lg font-semibold text-gray-900'>Kemaskini Maklumat Perbankan</p>
            ...
            <section className='mt-3'>
                <div className='flex justify-end items-center'>
                    <Button className='bg-teal-600 text-white' onClick={open_modal}>Kemaskini Maklumat Perbankan</Button>
                </div>
            </section>
        </div>
    );
...
export default KemaskiniBankInstitusi;